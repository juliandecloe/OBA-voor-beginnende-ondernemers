...
export function getstarterData() {
    ...
    .then(data => {
        let imageCheck = data.filter(element =>
            element.coverimages[0] === "https://v112.nbc.bibliotheek.nl/thumbnail?uri=http://data.bibliotheek.nl/ggc/ppn/172024889&token=c1322402";
        );
        console.log(imageCheck)
        renderstarterData(imageCheck);
    })
    .catch(err => {
        console.log(err);
    });
